...
class CreateAssetFromPO(models.TransientModel):
    ...
    def create_asset(self):
        for line in self.po_line_ids:
            # if line.assets_to_create <= 0:
            #     raise ValidationError(_("Zero or negative values detected."))
            #
            # if line.assets_to_create < len(line.employee_list):
            #     raise ValidationError(_("Number of employees is bigger then number of asset to create."))
            #
            # if line.assets_to_create > (line.received - line.asset_created):
            #     raise ValidationError(_("Cannot Create Asset, check number of asset created."))
            ...
            # line.po_line_id.asset_created_nb = line.po_line_id.asset_created_nb + 1
...
class CreateAssetFromPO(models.TransientModel):
    ...
    #     for rec in self:
    #         if rec.po_line_id.product_id and rec.parent_id.po_id.name:
    #             rec.name = rec.po_line_id.product_id.name + "-" + rec.parent_id.po_id.name
    #         else:
    #             rec.name = ""
    ...
    # employee_list = fields.Many2many(
    #     comodel_name='hr.employee',
    #     relation='ebs_po_emp_wiz_m2m',
    #     column1='rec_id',
    #     column2='emp_id',
    #     string='Employees',
    #     required=False)
